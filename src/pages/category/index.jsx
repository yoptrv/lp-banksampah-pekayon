...
import {
  kategoriSampahBase,
  mergeKategoriSampah,
} from "@/data/kategoriSampah";
import { RiHome2Fill } from "react-icons/ri";

export default function CategoryPage() {
  const [categories, setCategories] = useState(
    Object.values(mergeKategoriSampah(null)), 
  );
    ...
    const fetchPrices = async () => {
      try {
        const res = await fetch(
          "https://m0bnqlsf-5000.asse.devtunnels.ms/api/kategori-sampah",
        );
        ...
        // merge dan update
        const merged = mergeKategoriSampah(apiData);
        setCategories(Object.values(merged));
      } catch (err) {
        console.log("Gagal fetch API, pakai fallback '-'.");
      }
    };
            ...
            <div key={idx} className="space-y-8">
              ...
              <div className="flex flex-col md:flex-row md:items-center gap-6">
                <div className="relative w-full md:w-64 h-40 rounded-2xl overflow-hidden">
                  <Image
                    src={`/kategori/${category.title.toLowerCase().split(" ")[1]}.jpg`}
                    alt={category.title}
                    fill
                    className="object-cover"
                  />
